...
The generated trajectories are not empirical prevalence estimates, and the consultation model is not calibrated to a particular provider. Costs express planning priorities rather than monetary valuations. The report instead shows how an explicitly represented worried-well process can alter the timing, scale, and uncertainty buffer of consultation capacity, with each result linked to the implemented equations and saved scenario outputs.

## 2. Literature Review

### Worried-well behaviour as an operational phenomenon

The worried-well concept links psychological response with healthcare operations. Chatterjee et al. discuss its re-emergence during COVID-19 and explain why apparently healthy but concerned people can affect healthcare systems [@chatterjee_2020]. The relevant inference is that a non-infected population can generate service demand, so infection counts and consultation pressure need not coincide. The article motivates this demand channel but, as a conceptual clinical discussion rather than an optimization study, supplies neither the capacity rule nor the numerical service-seeking inputs used here.

Asmundson and Taylor place fear and anxiety within the context of an emerging outbreak, supporting the idea that perceived risk can influence behaviour [@asmundson_taylor_2020]. This perspective helps explain why worried-well prevalence can evolve alongside infection: concern may spread through social interaction and perceived exposure, then diminish as people leave the worried state. The resulting demand can arise earlier than an infection-only workload model suggests. The source does not specify a compartmental resource-planning model, consultation volumes, a stochastic demand distribution, or a capacity decision; these remain explicit modelling choices in this project.
...
Literature-derived elements are limited to the four-compartment structure, behavioural alpha regimes, baseline rates and initial condition, and the classical critical-fractile logic. The synthetic population size, consultation probabilities, coefficient of variation, mismatch costs, scenario counts, and sensitivity settings are coursework assumptions. The experiment therefore explains how capacity behaves *within the stated model*; a real schedule would require local estimation and a richer service representation.
...
Mean matching matters near the non-negative boundary because simply truncating a conventional normal distribution at zero would raise its mean above the requested value. Solving for the pre-truncation location avoids that shift. Stochastic integerization then produces count-valued requests without systematically rounding in one direction, while zero conditional mean is handled as zero demand. Scenario averages consequently target the demand equation while remaining non-negative and integer-valued.
...
The order-statistic rule minimizes the sample-average piecewise-linear loss by balancing the marginal consequences of shortage and unused capacity at the critical fractile. Because demand is integer-valued, adjacent capacities can have very similar costs. The selected in-sample quantile is therefore an operating target conditional on one generated sample, not a unique continuous optimum.
...
Out-of-sample evaluation reduces the risk of crediting a policy for random features of its optimization draw. The comparison is paired because the optimized and mean policies face identical evaluation scenarios. Finite-sample differences can still vary, especially under balanced costs when the rules select nearby capacities, so the reported signs and magnitudes are retained as generated.
...
Only the parameter named by an experiment group varies from its baseline, apart from the deliberate alpha crossing in the uncertainty group. This design supports attribution within the tested grid: differences between cost rows arise from costs, while differences between service-seeking rows arise from the demand mapping. Untested interactions remain outside the conclusions.

The design is comparative rather than inferential: it reports finite-sample expected performance within the implemented distribution and makes no confidence claim about a healthcare population. Numeric statements in the Results are linked to saved summary rows, separating generated evidence from assumptions that require local validation.

## 7. Results

### 7.1 Behavioural regimes

Behaviour changes both the height and timing of the two active compartments. In the cautious regime, the infected fraction peaks at [[VALUE:behaviour_alpha_0.5|peak_infected|.3f]] on day [[VALUE:behaviour_alpha_0.5|peak_infected_day|.0f]], while the worried-well fraction peaks earlier at [[VALUE:behaviour_alpha_0.5|peak_worried|.3f]] on day [[VALUE:behaviour_alpha_0.5|peak_worried_day|.0f]]. Under default behaviour, the corresponding infected peak is [[VALUE:behaviour_alpha_1|peak_infected|.3f]] on day [[VALUE:behaviour_alpha_1|peak_infected_day|.0f]], and under high contact it is [[VALUE:behaviour_alpha_2|peak_infected|.3f]] on day [[VALUE:behaviour_alpha_2|peak_infected_day|.0f]]. The curves compare timing horizontally and model fractions vertically. Higher alpha brings a larger infected peak forward while compressing the worried-well episode; lower alpha delays infected pressure and leaves a more persistent worried-well contribution. These comparisons concern deterministic scenario fractions, not measured prevalence.

[[FIGURE:figure_1_compartments.png|Compartment trajectories under cautious, default, and high-contact behaviour.|fig1]]

The worried-well-to-infected ratio clarifies why an infection-only staffing signal is incomplete. All three generated paths move above one early in the horizon, so the worried-well compartment temporarily exceeds the infected compartment. The cautious path remains above one longer and reaches the highest relative separation; the high-contact path crosses back sooner as infection accelerates. This conditional model ratio describes *composition*, not absolute workload or the observed mix of service users; consultation pressure also depends on population size and service-seeking probabilities.
...
In the right-hand cost-sensitivity panel, balanced underage and overage costs place the optimized quantile close to the mean benchmark for the modelled distribution. Increasing the underage penalty shifts the critical fractile upward, adding safety capacity and reducing exposure to shortage. The relative advantage of the optimized policy consequently grows: at \(c_u=10\), the signed relative reduction is [[VALUE:cost_cu_10_co_1|relative_cost_reduction_pct|.2f]] percent. This finite-sample comparison fixes \(c_o=1\) and assumes linear costs; it does not establish that the chosen ratios are economically correct.
...
For default behaviour and baseline costs, the optimized policy has aggregate out-of-sample cost [[VALUE:behaviour_alpha_1|optimized_total_cost|,.1f]] cost units, compared with [[VALUE:behaviour_alpha_1|mean_policy_total_cost|,.1f]] for rounded mean-demand planning. The signed relative reduction is [[VALUE:behaviour_alpha_1|relative_cost_reduction_pct|.2f]] percent. When shortage is more consequential than unused capacity, consultation targets should therefore sit above mean demand by an uncertainty buffer determined by the cost ratio.

[[POLICY_TABLE]]

The balanced-cost result is [[VALUE:cost_cu_1_co_1|relative_cost_reduction_pct|.4f]] percent and is negative: on the independent evaluation draw, the optimized finite-sample median policy performed very slightly worse than the rounded mean policy. The sign is retained rather than clamped to zero. Because the two rules are nearly equivalent under balanced costs, this result is consistent with sampling variation rather than a substantive disadvantage. At the higher shortage penalty, by contrast, the critical-fractile policy separates meaningfully from the mean. Overall, behaviour determines *when and how much* capacity is required, uncertainty determines the size and cost of mismatch, and the cost ratio determines how far capacity should move toward shortage protection.

## 8. Discussion

Daily consultation capacity should track the combined infected and worried-well demand process rather than the infected curve alone. Behavioural contact changes both the peak infected fraction and the timing and persistence of worried-well pressure. A cautious scenario can therefore require a lower but broader capacity response, while high contact requires an earlier and sharper response. The ratio figure shows that non-infected concern can dominate the modelled state composition early, although its contribution to demand is discounted by the baseline service probability.

Capacity should include an uncertainty buffer when shortage carries more weight than unused slots. Rather than adding a fixed number of places, the buffer scales with the day-specific distribution, whose spread is proportional to conditional mean under the implemented coefficient-of-variation model. Increasing uncertainty raises mismatch cost even after optimization. The shortage-to-overage ratio governs the appropriate service level: mean-demand planning is unsuitable under asymmetric costs, whereas balanced costs make the median-based and mean-based rules close enough for finite-sample noise to determine their ordering. The small negative balanced-cost result is retained to avoid overstating consistency.
...
All policy comparisons are finite-sample out-of-sample evidence. Fixed seeds make the reported experiment reproducible, not universal; alternative draws would produce small numerical changes, especially where competing rules are nearly equivalent.
...
Worried-well behaviour can materially change when capacity is needed, but the response depends on assumed service seeking, uncertainty, and costs. The model links behavioural compartments to a daily newsvendor decision; it is not a clinical schedule. Deployment would require empirical calibration, disaggregated resources, and multi-period constraints.
